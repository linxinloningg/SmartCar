...
void lcd_displayimage032_line(uint8 *p, uint16 width, uint16 height, int *leftline, int *centreline, int *rightline) {
    uint32 i, j;
    ...
    if (0 == TFT_DISPLAY_DIR || 1 == TFT_DISPLAY_DIR)//竖屏
    {
        ...
        for (j = 0; j < coord_y; j++) {
            ...
            for (i = 0; i < coord_x; i++) {
                ...
                if (i == centreline[j] || i == centreline[j] + 1)
                    lcd_writedata_16bit(GREEN);

                else if (i == leftline[j] || i == leftline[j] + 1)
                    lcd_writedata_16bit(BLUE);

                else if (i == rightline[j] || i == rightline[j] - 1)
                    lcd_writedata_16bit(RED);

                else lcd_writedata_16bit(color);
